...
function formatted(input){const raw=norm(input?.value);if(!raw)return 'Keine Eingabe';const number=Number(raw.replace(/\./g,'').replace(',','.'));if(!Number.isFinite(number))return raw;const id=input.id||'',pct=/Pct|Conversion|shift|percent/i.test(id),money=/^(otaRevenue|avgBooking|bbBookingValue)$/i.test(id),count=/^(bbBookings|bbContacts)$/i.test(id);return new Intl.NumberFormat('de-DE',{minimumFractionDigits:money?2:0,maximumFractionDigits:count?0:2}).format(number)+(pct?' %':money?' €':'')}
