...
const VERSION='HOTEL_BROCHURE_TEST_V3_PRINT_FIRST_8P';
...
function page6(data){
  ...
  let y=bigTitle(pg,'Was kann der Gutschein in Ihren Leistungen auslösen?',PH-132,W,25,4);y-=6;bodyText(pg,'Die folgenden Werte stammen direkt aus den Leistungsbeispielen Ihrer Hotel-Seite. So wird aus der Broschüre eine persönliche Arbeitsunterlage statt einer allgemeinen Präsentation.',y,W,10.2,5,C.gray);
...
function levelCard(pg,x,y,w,h,nr,value){roundRect(pg,x,y,w,h,12,C.soft,C.line,.5);text(pg,'Stufe '+nr,x+14,y+h-24,7.2,true,C.gray);text(pg,String(value)+' €',x+14,y+h-54,20,true,C.navy);text(pg,'Gutscheinwert',x+14,y+13,7.5,true,C.mag)}
function page7(){
  ...
  text(pg,'BEISPIELHAFTE STAFFELUNG',ML,263,7.7,true,C.mag);const lg=10,lw=(W-lg*3)/4;levelCard(pg,ML,176,lw,70,1,30);levelCard(pg,ML+lw+lg,176,lw,70,2,40);levelCard(pg,ML+2*(lw+lg),176,lw,70,3,50);levelCard(pg,ML+3*(lw+lg),176,lw,70,4,75);
...
function regionCard(pg,x,y,w,h,titleStr,price,bodyStr,accent){roundRect(pg,x,y,w,h,14,C.white,C.line,.65);fill(pg,x,y+h-5,w,5,accent);text(pg,titleStr,x+16,y+h-40,11.2,true,C.navy);text(pg,price,x+16,y+h-78,21,true,C.mag);lines(pg,bodyStr,x+16,y+h-102,w-32,8.9,11.8,false,C.gray,6)}
function page8(){
  ...
  callout(pg,105,102,'Mehr als ein einzelnes Hotelangebot','Hotel, Gastronomie, Freizeit und regionale Erlebnisse können gemeinsam einen größeren Erlebnisraum schaffen. Für die Broschüre bleibt dabei klar sichtbar: Partnerangebote gelten nur bei teilnehmenden Akzeptanzstellen.',C.mintSoft,C.mint,C.ink);footer(pg,8);return pg;
}
