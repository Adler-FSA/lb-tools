...
const VERSION='HOTEL_BROCHURE_TEST_V2_PRINT_FIRST';
...
function card(pg,x,y,w,h,titleStr,bodyStr,accent=C.mint,iconText=''){roundRect(pg,x,y,w,h,12,C.white,C.line,.65);fill(pg,x,y+h-5,w,5,accent);if(iconText){roundRect(pg,x+16,y+h-43,28,28,8,accent,null);text(pg,iconText,x+25,y+h-34,10,true,C.white)}text(pg,titleStr,x+16,y+h-63,11.2,true,C.navy);lines(pg,bodyStr,x+16,y+h-82,w-32,8.8,11.8,false,C.gray,6)}
...
function page1(data){
  ...
  const py=y-38;pill(pg,'Mehr Kontrolle',ML,py,108,C.white);pill(pg,'Mehr Wiederkehrer',ML+116,py,118,C.white);pill(pg,'Mehr Markenstärke',ML+242,py,124,C.white);pill(pg,'Mehr Datenhoheit',ML+374,py,112,C.white);
  const panelY=126,panelH=300;
  roundRect(pg,ML,panelY,W,panelH,18,C.white,C.line,1.0);fill(pg,ML,panelY+panelH-7,W,7,C.mint);fill(pg,ML,panelY,6,panelH,C.mag);
  text(pg,'IHRE PERSÖNLICHE ENTSCHEIDUNGSUNTERLAGE',ML+28,panelY+panelH-42,8.2,true,C.mag);
  lines(pg,data.house==='Ihr Haus'?'Für Ihr Haus':'Für '+data.house,ML+28,panelY+panelH-82,W-58,25,29,true,C.navy,2);
  lines(pg,'Direktbuchung, zusätzlicher Nutzwert und mehr Wirkung rund um den Aufenthalt.',ML+28,panelY+panelH-141,W-58,12.5,16,false,C.gray,4);
  rule(pg,ML+28,panelY+114,W-58,C.line,.8);
  text(pg,'DESIGN-TEST · PRINT-FIRST',ML+28,panelY+86,7.7,true,C.mintDark);text(pg,'Stand '+displayDate(),ML+28,panelY+60,9.5,false,C.gray);text(pg,'LiquidityBooster',ML+28,panelY+33,9.5,true,C.navy);
...
function page2(){
  ...
  callout(pg,160,130,'Die bessere Frage:','Wie wird aus Marketingaufwand ein eigener, wachsender Wert - und aus einem Gast mehr Nutzung rund um seinen Aufenthalt?',C.mintSoft,C.mint,C.ink);footer(pg,2,4);return pg;
}
function page3(){
  ...
  circle(pg,PW/2,cy+ch/2,25,C.navy);text(pg,'VS',PW/2-9,cy+ch/2-4,11,true,C.white);callout(pg,148,118,'Nicht billiger werden. Attraktiver werden.','Das ist die Grundidee hinter dem LiquidityBooster-Modell.',C.mintSoft,C.mint,C.ink);footer(pg,3,4);return pg;
}
...
function statCard(pg,x,y,w,h,label,value,accent){roundRect(pg,x,y,w,h,12,C.white,C.line,.7);fill(pg,x,y+h-5,w,5,accent);text(pg,value,x+16,y+48,20,true,C.navy);lines(pg,label,x+16,y+27,w-32,8.2,10.4,false,C.gray,2)}
function page4(){
  ...
  text(pg,'BEISPIEL AUS DER SEITE',ML,199,7.4,true,C.mag);
  const sg=18,sw=(W-sg*2)/3,sy=105,sh=78;statCard(pg,ML,sy,sw,sh,'reguläre Buchung','2.000 €',C.navy);statCard(pg,ML+sw+sg,sy,sw,sh,'Wertgutschein','400 €',C.mint);statCard(pg,ML+(sw+sg)*2,sy,sw,sh,'Reserveanteil im Beispiel','80 €',C.mag);
  arrow(pg,ML+sw+4,sy+39,ML+sw+sg-4,sy+39,C.mint);arrow(pg,ML+sw*2+sg+4,sy+39,ML+sw*2+sg*2-4,sy+39,C.mint);
...
function wireFrame(frame){
  const doc=frame.contentDocument;if(!doc)return;const panel=doc.querySelector('#pdfPanel'),oldBtn=doc.querySelector('#pdfBtn');
  if(panel){const label=panel.querySelector('.miniLabel'),h=panel.querySelector('.pdfCopy h3'),p=panel.querySelector('.pdfCopy p');if(label)label.textContent='PDF DESIGN-TEST · PRINT-FIRST';if(h)h.textContent='Druckfreundliche Hotel-Broschüre testen';if(p)p.textContent='Die Gestaltung ist jetzt auf hochwertigen Ausdruck optimiert: Weiß trägt, Navy/Mint/Magenta führen als Akzente. Die produktive Hotel-Seite bleibt unangetastet.'}
  if(oldBtn){const btn=oldBtn.cloneNode(true);btn.textContent='Druckfreundliche Broschüren-PDF erstellen';oldBtn.replaceWith(btn);btn.addEventListener('click',()=>{const old=btn.textContent;btn.disabled=true;btn.textContent='PDF wird erstellt …';setState(doc,'generating','<span class="pdfSpinner" aria-hidden="true"></span><div><strong>Die 4-seitige Print-first Test-PDF wird erstellt.</strong><div class="pdfHint">Weniger dunkle Flächen · gleiche Broschürenlogik</div></div>');try{const house=(doc.querySelector('#hotelName')?.value||'').trim()||'Ihr Haus',u8=buildBrochurePdf({house}),name=house==='Ihr Haus'?'Hotel_Broschuere_Print-Test_'+isoDate()+'.pdf':'Hotel_Broschuere_'+safeFilePart(house)+'_Print-Test_'+isoDate()+'.pdf';const blob=new Blob([u8],{type:'application/pdf'}),file=typeof File!=='undefined'?new File([blob],name,{type:'application/pdf'}):blob;if(pdfUrl)URL.revokeObjectURL(pdfUrl);pdfUrl=URL.createObjectURL(file);setState(doc,'success','<div class="pdfReadyTop"><span class="pdfCheck">✓</span><div><strong>Die Print-first Test-PDF ist fertig.</strong><div class="pdfHint">Vier Seiten mit deutlich reduziertem Farb- und Tintenverbrauch.</div></div></div><div class="pdfFile">'+escapeHtml(name)+'</div><a class="pdfDownload" href="'+pdfUrl+'" download="'+escapeHtml(name)+'" type="application/pdf" rel="noopener">PDF herunterladen / speichern</a>')}catch(err){console.error('Hotel brochure print test',err);setState(doc,'error','<strong>Die Test-PDF konnte nicht erstellt werden.</strong><div class="pdfHint">'+escapeHtml(err?.message||String(err))+'</div>')}finally{btn.disabled=false;btn.textContent=old}})}
...
global.HotelBrochureTestBuilder={version:VERSION,buildBrochurePdf,safeFilePart};if(typeof document!=='undefined')init();
