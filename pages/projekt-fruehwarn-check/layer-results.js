...
function setGlobalCoverage(kind='checking'){const result=document.querySelector('#matches-GLOBAL .regionResult');if(!result)return;if(document.querySelector('#matches-GLOBAL .match'))return;result.className='regionResult partial';if(kind==='match')result.innerHTML='<strong>Teilweise automatisch geprüft</strong>GLEIF: eindeutige Rechtsträger-/Identitätsspur gefunden. Globale Warnlisten sind derzeit überwiegend als Direktprüfung angebunden.';else if(kind==='ambiguous')result.innerHTML='<strong>Teilweise automatisch geprüft</strong>GLEIF: keine eindeutige Rechtsträger-Zuordnung vorgenommen. Ein ähnlicher Name oder eine abweichende Rechtsform reicht dafür nicht aus. Globale Warnlisten sind derzeit überwiegend als Direktprüfung angebunden.';else if(kind==='none')result.innerHTML='<strong>Teilweise automatisch geprüft</strong>GLEIF: Unternehmensidentität automatisch geprüft; kein eindeutiger LEI-Treffer. Globale Warnlisten sind derzeit überwiegend als Direktprüfung angebunden.';else if(kind==='error')result.innerHTML='<strong>Teilweise automatisch geprüft</strong>GLEIF ist als automatische Identitätsquelle angebunden, war bei dieser Prüfung jedoch nicht erreichbar. Globale Warnlisten bleiben teilweise Direktprüfung.';else result.innerHTML='<strong>Teilweise automatisch geprüft</strong>GLEIF prüft die Unternehmensidentität automatisch. Globale Warnlisten sind derzeit überwiegend als Direktprüfung angebunden.';}
...
if(exactUnique.length>1){const active=exactUnique.filter(x=>String(x?.item?.attributes?.entity?.status||'').toUpperCase()==='ACTIVE'&&String(x?.item?.attributes?.registration?.status||'').toUpperCase()==='ISSUED');if(active.length===1){const value={status:'match',record:normalizeGleif(active[0].item,active[0].term)};gleifCache.set(cacheKey,value);return value;}const value={status:'ambiguous',candidates:exactUnique.slice(0,5).map(x=>normalizeGleif(x.item,x.term))};gleifCache.set(cacheKey,value);return value;}
const fuzzyHits=[];
for(const term of terms){
try{
...
if(equivalents.length>1){const active=equivalents.filter(x=>String(x?.item?.attributes?.entity?.status||'').toUpperCase()==='ACTIVE'&&String(x?.item?.attributes?.registration?.status||'').toUpperCase()==='ISSUED');if(active.length===1){const value={status:'match',record:normalizeGleif(active[0].item,active[0].term)};gleifCache.set(cacheKey,value);return value;}const value={status:'ambiguous',candidates:equivalents.slice(0,5).map(x=>normalizeGleif(x.item,x.term))};gleifCache.set(cacheKey,value);return value;}
