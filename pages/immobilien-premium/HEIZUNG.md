...
**21.09.2026 · Baustein 2.3, Erweiterung Messbasis sowie bestätigte Nutzerwechsel. Begrenzter technischer Prototyp; keine Produkt-, Rechts- oder PDF-Freigabe.** Neu entwickelt; keine Übernahme aus dem alten Nebenkosten-Werkzeug.
...
- `assets/js/temporal.js`: optionale und ausschließlich durch das Heizmodul übergebene Umrechnung der **Verbrauchsdifferenz** per BigInt; zusätzliche dokumentierte Zwischenablesungen je Nutzerabschnitt sind nötig. Andere Berechnungen (insbesondere Kaltwasser) bleiben ohne solche Faktoren.
- **Neu `assets/js/thermal-user-change.js`:** `verifyThermalUserChange()` prüft die für jede Heiz- und Warmwasserart gesonderte Wahl; `splitThermalUnitShare()` verteilt erst vorhandene Wohnungs-Grund- und -Verbrauchskosten getrennt je dokumentierten Nutzungsabschnitt, einschließlich Leerstand. Kein Aufruf der Kaltwasser-Tages-Splittingfunktion.
- `tests/thermal.test.mjs`: aktualisierter Sperrtest; **neu `tests/thermal-user-change.test.mjs`: 10 positive/negative Tests mit fiktiven Mietverhältnissen und Leerstandszeiten**. Gesamtsuite: 139 lokale Tests erfolgreich, 0 Fehler. Browser, GitHub-CI und PDF-End-to-End nicht getestet.
...
4. Wohnungen mit lückenloser Nutzung und unveränderter Fläche; die Umlage verläuft pro Kostenposition in einen Verbrauchs- und einen Flächenanteil. Beide Anteile werden centgenau zuerst den Wohnungen zugerechnet. Beim Nutzerwechsel werden **diese beiden Anteile separat** auf Nutzerzeiten aufgeteilt. Eigennutzung und Leerstand verbleiben auf Eigentümerseite; sämtliche Mieter benötigen für ihre tatsächliche Nutzungsdauer passende Betriebskostenvereinbarungen.
   - `streams[].userChange:{confirmed:true,consumptionMethod:'intermediate_reading',baseMethod:'days'}` ist pro betroffener Dienstleistung erforderlich. Heizungs-Grundkosten dürfen alternativ ausdrücklich `baseMethod:'degree_days'` verwenden, allerdings nur mit `degreeDayWeightsBySegmentId` mit **vollständigem positivem Gewicht, bestätigtem Nachweis und Referenz pro Abschnitt**; diese Gewichte werden nicht aus erfundenen Wetterwerten erzeugt. Warmwasser-Grundkosten verwenden ausschließlich die bestätigte zeitanteilige Aufteilung.
   - Verbrauchskosten stützen sich auf exakt gemessene Zwischenwerte am Nutzerwechsel. Fehlende, widersprüchliche oder unbestätigte Zwischenwerte sperren; der Sonderfall ohne technisch ausreichende Zwischenablesung nach § 9b Abs. 3 HeizkostenV wird noch **nicht** berechnet. Bei nachgewiesenem Nullverbrauch kann der jeweilige Verbrauchskostenanteil null sein, Grundkosten werden dennoch separat verteilt.
   - Einzelne Wohnungs-Kostenanteile und individuelle Nutzungsanteile behalten separate Cent-Summenprüfungen. Jeder Mieter steht getrennt in `report.tenants[]`; `report.streams[].lines[].unitShares[]` enthält bei Wechsel die Nutzungsabschnittskennung, Basis, Werte und Teilbeträge.
...
- **Nutzerwechsel für den engen, belegten Standardfall jetzt technisch umgesetzt.** Weiterhin gesperrt: Wechsel ohne geeignete Zwischenablesung, fachlich notwendige Schätzung/Ersatzermittlung, abweichende rechtsgeschäftliche Bestimmungen, unbekannte Gradtaggewichte und unterjährige Flächenänderungen. Ein bestätigtes Gradtaggewicht ist ein Nutzereingabewert mit Referenz, keine automatisch geprüfte technische Norm.
...
**Nächste Unterstufe:** verbundene Wärme-/Warmwasseranlagen und die fachlich bestätigte Kostenabgrenzung; danach CO₂-Rechenweg. Die bestehende Rechenfunktion bleibt ein von der Standardabrechnung getrennter, nicht freigegebener Teilbericht. Das jetzt entwickelte Messbasis-Modul ist noch kein vollständiges Heizkostenabrechnungssystem. Baustein 2 bleibt offen.
