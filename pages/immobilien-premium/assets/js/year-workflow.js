...
export function runAnnualWorkflow(project, periodId, plans, engines) {
  ...
  try {
    const errors = engines.validate(project);
    ...
    if ((heating.length && !has(plans?.thermal)) || (!heating.length && has(plans?.thermal)) ||
        (linked && !has(plans?.linked)) || (!linked && has(plans?.linked)) ||
        (co2 && (!has(plans?.co2Building) || !has(plans?.co2Tenants))) ||
        (!co2 && (has(plans?.co2Building) || has(plans?.co2Tenants))))
    ...
    if (heating.length) {
      if (linked) {
        ...
        delete copy.supplyRegistry;
        delete copy.supplyRegistryVersion;
        thermal = engines.thermalLinked(copy, periodId, plans.linked, plans.thermal);
      }
      else {
        const copy = structuredClone(project);
        ...
        delete copy.supplyRegistry;
        delete copy.supplyRegistryVersion;
        thermal = engines.thermalSeparate(copy, periodId, plans.thermal);
      }
    ...
    if (co2) {
      ...
      carbon = engines.co2Tenants(project, periodId, plans.co2Building, thermal, plans.co2Tenants);
    ...
    let supply = null;
    const records = project.supplyRegistry?.filter(r => r.accountingPeriodId === periodId);
    if (records?.length) {
      ...
      supply = engines.supply(project, periodId, records);
    ...
    const audit = engines.audit(project, periodId, { standard, thermal, co2: carbon });
