...
function providerLabelFor(account){
  if(account.startsWith('demo_city_'))return 'Stadt Darmstadt · fiktiv';
  if(account.startsWith('demo_insurer_'))return 'MusterVersicherung AG · fiktiv';
  if(account.startsWith('demo_waste_provider_'))return 'Entsorgungsbetrieb Musterstadt · fiktiv';
  if(account.startsWith('demo_power_provider_'))return 'Stadtwerke Musterstadt · fiktiv';
  if(account.startsWith('demo_water_provider_'))return 'Stadtwerke Musterstadt · fiktiv';
  if(account.startsWith('demo_crafts_'))return 'Handwerksbetrieb Beispiel GmbH · fiktiv';
  return 'Demo-Anbieter · fiktiv';
}
function addProviderPayment(project,{id,year,providerAccountId,amountCents,date}){
  project.cashflows.push({
    ...
    providerLabel:providerLabelFor(providerAccountId),
...
function addYear2024(p){
  const costs=[
    ['demo_tax_2024','property_tax',240000,'demo_city_2024'],
    ['demo_insurance_2024','building_insurance',120000,'demo_insurer_2024'],
    ['demo_waste_2024','waste',72000,'demo_waste_provider_2024'],
    ['demo_power_2024','common_electricity',48000,'demo_power_provider_2024'],
...
function addYear2025(p){
  const costs=[
    ['demo_tax_2025','property_tax',252000,'demo_city_2025'],
    ['demo_insurance_2025','building_insurance',126000,'demo_insurer_2025'],
    ['demo_waste_2025','waste',75600,'demo_waste_provider_2025'],
    ['demo_power_2025','common_electricity',50400,'demo_power_provider_2025'],
...
function addSupplyRegistry(p){
  ...
  const serviceFor=category=>({
    common_electricity:'electricity',cold_water:'water',
    heating:'gas',hot_water:'gas',thermal_shared:'gas'
  })[category]||'other';

  for(const expense of p.expenses.filter(x=>x.providerAccountId)){
    const year=expense.startDate.slice(0,4);
    const isWater=expense.category==='cold_water';
    const planned=isWater?(year==='2024'?145:167):0;
    const workNumerator=isWater?expense.amountCents:0;
    const workDenominator=isWater?planned||1:1;
    const base=isWater?0:expense.amountCents;
    expense.supplyManaged=true;
    const short=expense.id.replace(/^demo_/,'').replace(/_20\d\d$/,'');
    p.supplyRegistry.push({
      id:isWater?`demo_supply_water_${year}`:`demo_supply_${short}_${year}`,
      propertyId:'demo_house',accountingPeriodId:`demo_year_${year}`,confirmed:true,
      contract:{
        providerLabel:providerLabelFor(expense.providerAccountId),
        providerAccountId:expense.providerAccountId,
        service:serviceFor(expense.category),
        contractHolder:'owner',confirmed:true,
        priceVersions:[{
          validFrom:`${year}-01-01`,validTo:`${year}-12-31`,confirmed:true,
          referenceId:`DEMO-PREIS-${expense.category.toUpperCase()}-${year}`,
          baseCentsPerPeriod:base,plannedWholeUnits:planned,
          workPriceNumeratorCents:workNumerator,workPriceDenominatorUnits:workDenominator,
          measurementUnit:isWater?'m³':'Jahr'
        }],
        expenseIds:[expense.id],
        invoiceTotalsCentsByReference:{[expense.invoiceReference]:expense.amountCents}
      }
    });
  }
}
