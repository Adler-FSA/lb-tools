...
function ensureCoreConfirmations(){const root=document.getElementById('acceptChecks');if(!root)return;const currentLang=lang(),t=texts[currentLang];let wrap=root.querySelector('.confirmation-core');if(wrap&&wrap.dataset.lang===currentLang&&wrap.querySelectorAll('.coreAcceptCheck').length===t.confirmations.length)return;if(!wrap){wrap=document.createElement('div');wrap.className='confirmation-core';root.prepend(wrap)}wrap.dataset.lang=currentLang;wrap.innerHTML='';t.confirmations.forEach((txt,i)=>{const label=document.createElement('label');label.className='check';label.innerHTML='<input type="checkbox" class="acceptCheck coreAcceptCheck" data-core-index="'+i+'"><span>'+txt+'</span>';wrap.appendChild(label)})}
...
function init(){installStyle();syncAll();const root=document.getElementById('acceptChecks');if(root)new MutationObserver(()=>{if(!root.querySelector('.confirmation-core')){ensureCoreConfirmations();syncCopy()}}).observe(root,{childList:true});const state=document.getElementById('downloadState'),src=document.getElementById('downloadLink'),info=document.getElementById('downloadInfo');const obs=new MutationObserver(mirrorDownload);if(state)obs.observe(state,{attributes:true,attributeFilter:['class']});if(src)obs.observe(src,{attributes:true,attributeFilter:['href','download']});if(info)obs.observe(info,{childList:true,characterData:true,subtree:true});document.querySelectorAll('[data-lang]').forEach(b=>b.addEventListener('click',()=>setTimeout(syncAll,0)));document.getElementById('resetBtn')?.addEventListener('click',()=>setTimeout(()=>{document.getElementById('mainDownloadCard')?.classList.remove('show');syncAll()},0))}
