...
# Five result cards on wide screens; responsive rules already reduce them later
s=s.replace(".results{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-top:24px}",".results{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-top:24px}",1)
...
oldacc="empCum+=empMonth;lbCum+=lbMonth;lbRecurringCum+=recurring;"
newacc="empCum+=empMonth;lbCum+=lbMonth;lbRecurringCum+=recurring;lbSetupCum+=setupPerMonth;"
if oldacc not in s: raise SystemExit('accumulator not found')
s=s.replace(oldacc,newacc,1)

oldout="$('empTotal').textContent=eur(empCum);$('lbTotal').textContent=eur(lbCum);$('lbRecurring').textContent=eur(lbRecurringCum);$('difference').textContent=(lbCum-empCum>=0?'+':'')+eur(lbCum-empCum);"
newout="$('empTotal').textContent=eur(empCum);$('lbTotal').textContent=eur(lbCum);$('lbRecurring').textContent=eur(lbRecurringCum);$('lbSetup').textContent=eur(lbSetupCum);$('difference').textContent=(lbCum-empCum>=0?'+':'')+eur(lbCum-empCum);"
if oldout not in s: raise SystemExit('results output not found')
s=s.replace(oldout,newout,1)
