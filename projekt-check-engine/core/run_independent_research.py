...
GENERIC_BRAND_WORDS = {
    "U","CENTER","CENTRE","HOME","TERMS","PRIVACY","POLICY","CONDITIONS","SIGN","OPEN","THE","LEARNING",
    "COMMUNITY","PLATFORM","PACKAGE","PRICE","STARTER","BRONZE","SILVER","GOLD","PLATINUM","DIAMOND","LEGAL",
}
...
def extract_distinctive_terms(primary_evidence: dict, label: str) -> list[str]:
    ...
    for raw in re.findall(r"\b[A-Z][A-Z0-9]{0,20}(?:[- ][A-Z][A-Z0-9]{1,20})+\b", text):
        ...
        if len(term) < 4 or words <= GENERIC_BRAND_WORDS:
            continue
        if label_key and identity_key(term) == label_key:
            continue
        # Markante Bindestrich-Marken wie U-TOPIA werden bewusst höher gewichtet.
        score = 6 if "-" in term else 1
...
def current_independent_ref_counts(evaluation: dict) -> Counter:
    counts = Counter()
    for check in evaluation.get("checks") or []:
        cid = int(check.get("id") or 0)
        if not cid:
            continue
        refs = ((check.get("neutral_finding") or {}).get("evidence_refs") or [])
        counts[cid] = sum(1 for r in refs if str(r).startswith("W"))
    return counts
...
def strip_previous_independent(evaluation: dict) -> None:
    for check in evaluation.get("checks") or []:
        ...
        nf["confirmed_facts"] = [
            x for x in (nf.get("confirmed_facts") or [])
            if "unabhängige Web-Recherche" not in str(x)
            and "thematische unabhängige Websuchen" not in str(x)
            and "identitätsgebundene unabhängige Web-Recherche" not in str(x)
        ]
...
def main() -> int:
    ...
    old_w_counts = current_independent_ref_counts(evaluation)
    ...
    independent={
        "schema_version":"1.2","case_id":case_id,"started_at":started,"finished_at":now(),
        "search_query_count":len(search.get("queries") or []),"search_result_count":len(results),
        "rejected_result_count":len(rejected),"external_result_count":len(external),"capture_count":len(items),
        "search_errors":search.get("errors") or [],"items":items
    }
    research={
        "schema_version":"1.2","case_id":case_id,"identity_label":label,"project_hosts":sorted(project_hosts),
        "distinctive_terms":distinctive_terms,"queries":search.get("queries") or [],"results":results,
        "rejected_results":rejected[:40],"selected_urls":[x.get("url") for x in selected],
        "errors":search.get("errors") or [],"finished_at":now(),
        "note":"Suchtreffer werden nur übernommen, wenn Domain oder ein markantes Projektmerkmal die Zuordnung stützt. Suchtreffer sind Recherchehinweise; Belege sind separat geöffnete Zielseiten."
    }
    write(case_dir/"independent-research.json",research); write(case_dir/"independent-evidence.json",independent)

    new_counts=Counter(); refs_by_check={}
    for item in items:
        if item.get("error"):
            continue
    ...
    for cid in set(old_w_counts)|set(new_counts):
        st=st_by.get(cid); ev=ev_by.get(cid)
        if not st or not ev:
            continue
        st["evidence_count"]=max(0,int(st.get("evidence_count") or 0)-old_w_counts.get(cid,0))+new_counts.get(cid,0)
        if new_counts.get(cid,0) and cid!=20 and st.get("workflow_status")!="abgeschlossen":
            ...
            for p in ("customer","company","academy"):
                st["perspectives"][p].update({"status":"laeuft","updated_at":ts})
    ...
    if search_executed:
        st20["workflow_status"]="abgeschlossen"; st20["result_status"]="bestaetigt" if usable else "kein_befund"
        ...
        for p in ("customer","company","academy"):
            st20["perspectives"][p].update({"status":"abgeschlossen","updated_at":ts})
    ...
    write(case_dir/"research-progress.json",{
        "schema_version":"1.2","case_id":case_id,"updated_at":ts,
        "modules":{"primary_discovery":"completed","primary_evaluation":"completed","independent_web":"completed" if search_executed else "partial"},
        "independent_search_results":len(results),"independent_rejected_results":len(rejected),"independent_captures":len(usable)
    })
    print(json.dumps({"case_id":case_id,"distinctive_terms":distinctive_terms,"results":len(results),"rejected":len(rejected),"captures":len(usable),"check20":st20["workflow_status"]},ensure_ascii=False))
    return 0
