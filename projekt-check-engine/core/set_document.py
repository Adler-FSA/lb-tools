...
def main() -> int:
    ap = argparse.ArgumentParser(description="Aktualisiert den Status eines der zwei Projekt-Check-PDFs.")
    ap.add_argument("--status-file", required=True, type=Path)
    ap.add_argument("--document", required=True, choices=["user_check", "full_analysis"])
    ap.add_argument("--status", required=True, choices=["wartet", "wird_erstellt", "fertig", "fehler"])
