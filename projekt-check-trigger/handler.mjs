...
export async function handleRequest(request, env = {}) {
  ...
  if (!githubToken) return reply({accepted:false,error:'trigger_not_configured'}, 503, allowedOrigin);

  let payload;
  try {
    payload = validatePayload(await request.json());
  } catch (error) {
    return reply({accepted:false,error:String(error?.message || 'invalid_payload')}, 400, allowedOrigin);
  }
  ...
  const response = await fetch(`https://api.github.com/repos/${repository}/dispatches`, {
    ...
    headers: {
      ...
      'user-agent': 'FSA-Projekt-Check-Trigger/1.0'
    },
    body: JSON.stringify({
  ...
  if (!response.ok) {
    return reply({accepted:false,error:'github_dispatch_failed'}, 502, allowedOrigin);
  }
