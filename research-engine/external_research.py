...
def _result_from_anchor(anchor, snippet: str, query: str, provider: str) -> SearchHit | None:
    ...
    if not host or host in SEARCH_BLOCKLIST:
        return None
    return SearchHit(canonical_url(href), clean_text(anchor.get_text(" ")), clean_text(snippet), query, provider)
...
def search_duckduckgo(query: str, limit: int = MAX_RESULTS_PER_QUERY) -> list[SearchHit]:
    try:
        r = requests.get("https://html.duckduckgo.com/html/?q=" + quote_plus(query), headers={"User-Agent": UA, "Accept": "text/html"}, timeout=TIMEOUT)
        if not r.ok:
            return []
...
def search_bing(query: str, limit: int = MAX_RESULTS_PER_QUERY) -> list[SearchHit]:
    try:
        r = requests.get("https://www.bing.com/search", params={"q": query, "count": limit}, headers={"User-Agent": UA, "Accept": "text/html"}, timeout=TIMEOUT)
        if not r.ok:
            return []
        soup = BeautifulSoup(r.text, "html.parser")
        out: list[SearchHit] = []
...
def web_search(query: str, limit: int = MAX_RESULTS_PER_QUERY) -> tuple[list[SearchHit], list[dict]]:
    attempts = []
    for provider, fn in (("bing-rss", search_bing_rss), ("duckduckgo", search_duckduckgo), ("bing", search_bing)):
        hits = fn(query, limit)
...
def extract_published_at(soup: BeautifulSoup, raw_text: str = "") -> str:
    ...
    for script in soup.select('script[type="application/ld+json"]'):
        m = re.search(r'"datePublished"\s*:\s*"([^"]+)"', script.get_text(" "), re.I)
        if m:
...
def read_public_page(url: str) -> dict:
    try:
        ...
        if r.ok and ("html" in r.headers.get("content-type", "") or "text" in r.headers.get("content-type", "")):
            source_soup = BeautifulSoup(r.text, "html.parser")
            soup = BeautifulSoup(r.text, "html.parser")
            ...
            if len(text) >= 220:
                return {"ok": True, "url": canonical_url(r.url or url), "title": title, "text": text[:50000], "published_at": extract_published_at(source_soup, r.text), "mode": "direct"}
    except requests.RequestException:
        pass
    try:
        r = requests.get("https://r.jina.ai/" + url, headers={"User-Agent": UA, "Accept": "text/plain"}, timeout=TIMEOUT)
        if r.ok and len(r.text) >= 300:
            ...
            if len(text) >= 180:
                return {"ok": True, "url": canonical_url(url), "title": clean_text(title_match.group(1)) if title_match else "", "text": text[:50000], "published_at": clean_text(published_match.group(1))[:40] if published_match else "", "mode": "reader-fallback"}
    except requests.RequestException:
        pass
...
def relation_for(url: str, project_domain: str) -> tuple[str, str, str]:
    ...
    for platform_host, (category, platform) in PLATFORM_HOSTS.items():
        if host == platform_host or host.endswith("." + platform_host):
            return ("community" if category == "community" else "platform"), category, platform
...
def evidence_snippet(text: str, project_name: str, domain: str, fallback: str, width: int = 360) -> str:
    ...
    low = text.lower()
    for needle in [clean_text(project_name), domain]:
        if needle:
            idx = low.find(needle.lower())
            if idx >= 0:
                return clean_text(text[max(0, idx - width // 3):min(len(text), idx + 2 * width // 3)])[:width]
...
def query_plan(project_name: str, domain: str, legal_entities: Iterable[str]) -> list[tuple[str, str]]:
    ...
    plan = [
        ("article", qname), ("article", f'{qname} crypto OR investment OR yield'),
        ("video", f'{qname} site:youtube.com'), ("social", f'{qname} site:facebook.com'),
        ("social", f'{qname} site:instagram.com'), ("social", f'{qname} site:tiktok.com'),
        ("social", f'{qname} site:x.com OR site:twitter.com'), ("social", f'{qname} site:linkedin.com'),
        ("social", f'{qname} site:t.me'), ("community", f'{qname} review OR experience OR erfahrung'),
        ("community", f'{qname} withdrawal OR payout OR auszahlung'), ("community", f'{qname} site:reddit.com'),
        ("operator", f'{qname} founder OR CEO OR director OR owner'), ("operator", f'"{domain}" company OR operator OR company register'),
    ]
...
def _dedupe_traces(traces: list[ExternalTrace]) -> list[ExternalTrace]:
    ...
    out.sort(key=lambda t: (0 if t.source_relation == "independent" else 1, {"article": 0, "video": 1, "community": 2, "social": 3, "operator": 4}.get(t.category, 9), t.title.lower()))
    return out
...
def enrich(core_result: dict) -> dict:
    ...
    if not project_name or not domain:
        result["external_research"] = {"status": "not_ready", "generated_at": datetime.now(timezone.utc).isoformat(), "reason": "Projektname oder bestätigte Projektdomain fehlt.", "traces": [], "review_candidates": [], "project_owned_echoes": [], "search_attempts": []}
        return result
    ...
    for requested_category, query in query_plan(project_name, domain, legal_entities):
        ...
        for hit in hits:
            ...
            category = detected_category if detected_category in {"video", "social", "community"} else requested_category
            ...
            collected.append(ExternalTrace(category, relation, platform, page.get("url") or url, title, evidence, clean_text(page.get("published_at") or ""), confidence, project_match, f"{hit.provider}: {query}", bool(page.get("ok"))))

    deduped = _dedupe_traces(collected)
    project_owned_echoes = [t for t in deduped if t.attribution_confidence == "high" and t.source_relation == "project_owned"]
    final_traces = [t for t in deduped if t.attribution_confidence == "high" and t.source_relation != "project_owned"]
    ...
    result["external_research"] = {
        ...
        "counts_by_category": counts, "counts_by_relation": relations,
        "review_candidate_count": len(review_candidates), "project_owned_echo_count": len(project_owned_echoes),
...
def main() -> int:
    ...
    out.write_text(json.dumps(enrich(source), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(out)
    return 0
